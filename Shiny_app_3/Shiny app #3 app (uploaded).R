...
Countries_cr <- select(global_time_series, Country_Region) 
Countries <- filter(Countries_cr, Country_Region == c("China","Italy","Canada", "US", "Spain"))
    ...
    sidebarLayout(
        sidebarPanel(
            ...
            selectInput("select_type", 
                        ...
                        selected = "Confirmed"),
            # Date range
            sliderInput("range", label = "Report Date", min = first_date, 
                        max = last_date, value = c(first_date,last_date))
        ),
...
server <- function(input, output) {
    ...
    output$Plot1 <- renderPlot({
        ...
        ggplot(pick_country, aes_string(x = "Date",  y = input$select_type, color = "Country_Region")) + 
            ...
            ggtitle("Frequencies of Confirmed, Deaths and Recovered Cases:", input$select_type)
    })
}
